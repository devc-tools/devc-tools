...
check "subuid: vscode gets the in-namespace range, the 100000 one is gone" bash -c \
  "grep -qxF 'vscode:10000:50001' '$subuid' && ! grep -q '100000' '$subuid'"
check "subuid: the placeholder (holder of uid 1000) too" grep -qxF 'devc-uid-hold:10000:50001' "$subuid"
check "subgid mirrors it" bash -c "grep -qxF 'vscode:10000:50001' '$subgid' && grep -qxF 'devc-uid-hold:10000:50001' '$subgid'"
...
check "  subuid covers vscode and the placeholder" bash -c \
  "grep -qxF 'vscode:10000:50001' '$subuid' && grep -qxF 'devc-uid-hold:10000:50001' '$subuid'"
...
check "  subuid has exactly one line per name" bash -c "[ \"\$(grep -c '^vscode:' '$subuid')\" -eq 1 ] && [ \"\$(grep -c '^devc-uid-hold:' '$subuid')\" -eq 1 ]"
