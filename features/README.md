...
| [podman-as-docker](podman-as-docker/README.md)         | `ghcr.io/devc-tools/features/podman-as-docker`     | Makes `docker` commands work via Podman and the podman-docker shim — no Docker daemon, no privileged sibling container. **Not the secure option**: costs `CAP_SYS_ADMIN` and `systempaths=unconfined`, both declared unconditionally. Not on the publish allowlist yet.               |
