...
Left unset, `devcontainer features test`'s own `--base-image`/`-i` defaults to
`ubuntu:focal` (20.04, no `curl` preinstalled) — but that flag only ever reaches the
autogenerated default scenario (`test.sh`); every named scenario in `scenarios.json` pins
its own `"image"` regardless. The wrapper defaults it instead, unless the caller already
passed one, to `mcr.microsoft.com/devcontainers/base:ubuntu`, the floating tag every named
scenario in this collection already uses — so `test.sh` builds against the same base a
Feature is actually meant to run on. Pass `--base-image`/`-i` yourself to override it, the
way `test-podman-as-docker.yml`'s default-scenario job does.
